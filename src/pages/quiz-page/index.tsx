...
function QuizPage() {
  ...
  const confirmAnswer = () => {
    ...
    newAnswers[currentQuestion] = selectedAnswer == questions[currentQuestion].correctAnswer
  ...
  const currentQuestionTheme = getQuestionTheme(questions[currentQuestion].theme)
  ...
  return (
    <div className='w-full flex flex-col items-center justify-start relative'>
      <div className='flex text-white bg-blueGradient w-full h-28 justify-center'>
        ...
        <Timer totalDuration={30} handleTimeOver={handleTimeOver}/>
        </div>
      </div>
      <div className='bg-white w-2/4 h-max p-8 rounded-sm absolute top-20 shadow-cardShadow'>
        <div className='flex flex-col gap-4 sm:gap-2 lg:gap-6 xl:gap-8 2xl:gap-12'>
          <div className='flex flex-col px-4 gap-2'>
            <div className='flex items-center justify-between'>
                ...
                <div className='flex gap-1'>
                    {currentQuestionTheme ?
                        <div className={`${currentQuestionTheme.color} rounded-md px-2`}>
                            <p className='text-white text-sm font-semibold'>{currentQuestionTheme.name}</p>
                        </div>
                        :
                        <></>
                    }
            ...
            <h1 className="text-gray-500 text-2xl leading-7">{questions[currentQuestion].question}</h1>
          </div>
          <div className='flex flex-col gap-1'>
            ...
            <div>
              {questions[currentQuestion].options.map((answer: AnswerOption) => {
                return(
                  <QuizOption 
                    key={answer.id}
                    index={answer.id} 
